...
numeric_feature_cols = [c for c in data.select_dtypes(include=[np.number]).columns.tolist() if c != TARGET_COLUMN]
...
print('\nTraining models...\n')
models = {'Linear Regression': LinearRegression(), 'Decision Tree': DecisionTreeRegressor(max_depth=10, random_state=RANDOM_STATE), 'Random Forest': RandomForestRegressor(n_estimators=20, max_depth=10, random_state=RANDOM_STATE)}
results = {}
for (name, model) in models.items():
    ...
    results[name] = {'model': model, 'MAE': round(mae, 4), 'RMSE': round(rmse, 4), 'R2': round(r2, 4)}
...
metrics_to_save = {name: {k: v for (k, v) in r.items() if k != 'model'} for (name, r) in results.items()}
