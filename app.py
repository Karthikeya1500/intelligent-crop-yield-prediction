...
Streamlit web application — Intelligent Crop Yield Prediction &
Agentic Farm Advisory System (Milestone 1 + 2).

Pages:
  1. Predict Yield      — ML-based yield prediction
  2. Farm Advisory      — Agentic AI advisory report
  3. Data Explorer      — Dataset exploration
  4. Model Performance  — Evaluation metrics & feature importance
...
# ---------------------------------------------------------------------------
st.set_page_config(
    page_title="Intelligent Crop Yield Prediction & Advisory",
    page_icon=None,  # intentionally removing emoji — cleaner for professional use
    layout="wide",
...
# Not shown to users: farmers should not have to manage API keys
# For deployment, add GEMINI_API_KEY to your hosting provider secrets dashboard
...
if page == "Predict Yield":
    ...
    with col1:
        ...
        selected_year = st.number_input(
            "Year", min_value=1960, max_value=2030, value=2024, step=1, key="p1_year"
        )
...
# ===========================================================================
# PAGE 2: FARM ADVISORY
# ===========================================================================
elif page == "Farm Advisory":
    st.markdown('<p class="main-header">Agentic Farm Advisory System</p>', unsafe_allow_html=True)
    st.markdown(
        '<p class="sub-header">AI-powered crop management recommendations based on yield prediction & risk analysis</p>',
        unsafe_allow_html=True,
    )
    st.divider()
    ...
    with st.expander("Farm Input Parameters", expanded=True):
        ...
        with col1:
            ...
            adv_year = st.number_input(
                "Year", min_value=1960, max_value=2030,
                value=int(last.get("year", 2024)), step=1, key="adv_year"
            )
        ...
        with col2:
            ...
            adv_rainfall = st.number_input(
                "Average Rainfall (mm/year)", min_value=0.0, max_value=5000.0,
                value=float(last.get("rainfall", 1000.0)), step=10.0, key="adv_rain"
            )
            adv_pesticides = st.number_input(
                "Pesticides Used (tonnes)", min_value=0.0, max_value=500000.0,
                value=float(last.get("pesticides", 1000.0)), step=100.0, key="adv_pest"
            )
            adv_temp = st.number_input(
                "Average Temperature (C)", min_value=-10.0, max_value=50.0,
                value=float(last.get("avg_temp", 25.0)), step=0.5, key="adv_temp"
            )

    run_advisory = st.button(
        "Generate Farm Advisory Report", use_container_width=True, key="btn_advisory"
    )
    ...
    if run_advisory:
        ...
        risks = analyze_risks(
            crop=adv_crop, area=adv_area, year=adv_year,
            rainfall=adv_rainfall, temperature=adv_temp,
            pesticides=adv_pesticides, predicted_yield=predicted_yield,
        ...
        status_text.markdown(
            '<span class="step-badge">1</span> **Yield Prediction** — ML model complete',
            unsafe_allow_html=True)
        progress_bar.progress(25)

        status_text.markdown(
            '<span class="step-badge">2</span> **Risk Analysis** — Evaluating farm conditions...',
            unsafe_allow_html=True)
        progress_bar.progress(50)

        status_text.markdown(
            '<span class="step-badge">3</span> **Knowledge Retrieval** — Fetching agronomy guidelines...',
            unsafe_allow_html=True)
        progress_bar.progress(75)

        status_text.markdown(
            '<span class="step-badge">4</span> **Report Generation** — '
            + ("Calling Gemini AI..." if gemini_api_key else "Building rule-based advisory..."),
            unsafe_allow_html=True)

        # Step 4: Run Advisory Agent
        ...
        result = run_advisory_agent(
            ...
            rainfall=adv_rainfall, temperature=adv_temp,
            pesticides=adv_pesticides, predicted_yield=predicted_yield,
            feature_importance=feature_importance, risks=risks,
        ...
        badge_class = {"Low": "yield-badge-low", "Medium": "yield-badge-medium",
                       "High": "yield-badge-high"}.get(yield_cat, "yield-badge-medium")

        with c1: st.metric("Crop", adv_crop)
        with c2: st.metric("Region", adv_area)
        with c3: st.metric("Predicted Yield", f"{predicted_yield:,.0f} hg/ha")
        with c4:
            st.markdown(
                f"**Yield Category**<br><span class='{badge_class}'>{yield_cat}</span>",
                unsafe_allow_html=True)
        ...
        for risk in sorted_risks:
            ...
            st.markdown(
                ...
                f'<strong>[{risk["severity"]}] {risk["factor"]}</strong><br>'
                f'{risk["description"]}</div>',
                unsafe_allow_html=True)
        ...
        with export_col1:
            md_content = (
                ...
                f"**Predicted Yield:** {predicted_yield:,.0f} hg/ha ({yield_cat})\n\n---\n\n"
                f"{result['report']}\n"
            )
            st.download_button(
                label="Download as Markdown (.md)",
                data=md_content.encode("utf-8"),
                file_name=f"farm_advisory_{adv_crop.replace(' ', '_')}_{adv_year}.md",
                mime="text/markdown",
        ...
        with export_col2:
            ...
            st.download_button(
                ...
                file_name=f"farm_advisory_{adv_crop.replace(' ', '_')}_{adv_year}.txt",
                mime="text/plain",
...
# ===========================================================================
    ...
    with o1: st.metric("Rows", df.shape[0])
    with o2: st.metric("Columns", df.shape[1])
    with o3: st.metric("Missing Values", int(df.isnull().sum().sum()))
    with o4: st.metric("Numeric Cols", len(df.select_dtypes(include=[np.number]).columns))
...
# ===========================================================================
    ...
    for bar, val in zip(bars, comparison_df["R2 Score"]):
        ax_r2.text(bar.get_width() + 0.003, bar.get_y() + bar.get_height() / 2,
...
st.divider()
st.markdown(
    "<center><small>Intelligent Crop Yield Prediction &amp; Agentic Farm Advisory System "
    "&nbsp;|&nbsp; Powered by scikit-learn &amp; Google Gemini</small></center>",
    unsafe_allow_html=True,
)
